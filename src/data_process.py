...
for i in xrange(0,num_files):
    ...
    if file_labels[i] != TESTING_LABEL:   
        ...
        if file_labels[i] == TRAINING_LABEL:
            ...
            trainOriginalImage.append(im_file_names[i])
...
scipy.io.savemat(arguments.output_file,{'trainOriginalImage':trainOriginalImage,'train_edges':train_edges,'train_edgesFeatures1':train_edgesFeatures1,'train_edgesFeatures2':train_edgesFeatures2,'valid_edges':valid_edges,'valid_edgesFeatures1':valid_edgesFeatures1,'valid_edgesFeatures2':valid_edgesFeatures2,'train_data':train_data, 'valid_data':valid_data, 'train_labels':train_labels, 'valid_labels':valid_labels, 'file_labels':file_labels, 'im_file_names':im_file_names, 'sp_file_names':sp_file_names, 'label_file_names':label_file_names,'valid_pixels_labels':valid_pixels_labels,'valid_files':valid_files,'valid_files_count':valid_files_count,'valid_superpixels':valid_superpixels,'test_files_count':test_files_count,'validationOriginalImage':validationOriginalImage,'train_superpixels':train_superpixels}, oned_as='column')
scipy.io.savemat(arguments.test_file,{'test_pixels_labels':test_pixels_labels,'test_files':test_files,'testOriginalImage':testOriginalImage,'test_superpixels':test_superpixels,'test_data':test_data,'test_label':test_labels,'test_edges':test_edges,'test_edgesFeatures1':test_edgesFeatures1,'test_edgesFeatures2':test_edgesFeatures2},oned_as='column')
