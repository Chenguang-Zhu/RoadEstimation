...
print clf.score(train_data,train_labels.ravel())
#scores.mean()
"""
#plot black-white pics
#print data['train_superpixels'][0]
print data['valid_superpixels'].shape[0]
print data['valid_superpixels'].shape[1]
print data['valid_superpixels'][0][0].shape[0]
print data['valid_superpixels'][0][0].shape[1]

#for i in range(data['train_superpixels'].shape[0]):
for i in range(0,1):
    predict_img = data['valid_superpixels'][i][0]
    for j in range(predict_img.shape[0]):
        for k in range(predict_img.shape[1]):
            current_sp = predict_img[j][k]
            if predict[i + current_sp][1] > 0.5:
                #predict_img[j][k] = predict[i + current_sp][1] * 255
                predict_img[j][k] = 1 * 255
                #print "hit 1"
            else:
                #predict_img[j][k] = predict[i + current_sp][1] * 255
                predict_img[j][k] = 0 * 255
                #print "hit 0" 
    #io.imsave('/Users/echo/Desktop/chen/beforeCRF.png', predict_img)
"""
